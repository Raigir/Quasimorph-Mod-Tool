...
| traitEffects.txt | `config_items_properties.txt` `#itemtraits` section | The `Parameters` column holds one or two "Effect modifier" token pairs (modifier may be float/int/string; not collected). Every effect name is collected uniquely into `Name`. Effects that only ever appear as a second pair are non-functional without their primary; `DependentTrait` lists the primary effect(s) they appeared under, space-separated. Effects appearing as a primary anywhere get a blank `DependentTrait`. |
