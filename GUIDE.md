...
**Disassembly** — Dynamic entry list, with **+ Add Entry** and **Copy Craft** buttons plus an info icon ("Weapons usually match their crafting recipe with their disassembly list."). **Copy Craft** replaces the whole disassembly list with the Crafting Recipe tab's Required Items, carrying each item's count across — the one-click version of the usual case. It overwrites anything already in the list; if the recipe has no items it does nothing rather than clearing your work. (The former CanDisassembly toggle and JSON key are gone — the game derives it from whether the disassembly list has entries. Older weapon files carrying the key are cleaned automatically on their next save.) Each entry has an Item Id (combo box with suggestions from repairs + trash TSVs, `quest_` items filtered out, allows free text) and a Count (integer > 0). Click **+ Add Entry** to add rows, **×** to remove.
...
A switch at the top of the tab controls whether a recipe exists at all. Its state isn't stored anywhere — it simply reflects whether the receipt file is present, so nothing needs migrating. **New weapons default to on** and get a receipt created with them; **new ammo defaults to off** and gets no receipt until the switch is flipped and the ammo saved. Copying mirrors the source: if the source had a receipt the copy gets one, if it didn't the copy doesn't.

Turning the switch off and saving **deletes the receipt file**. The fields stay on screen and editable so you can see what you'd be re-enabling, but they aren't written anywhere and their values are lost once you leave the item. Fields also stop blocking the save while the switch is off — a blank Produce Time only matters when a recipe is actually being written.

**Production Settings** — ProduceTimeInHours (number > 0 — floats are fine), plus a Required Items entry list (max 7, with an info icon beside the Add button). Weapon receipts save to `Crafting Recipes/Weapons/`. Each entry has an Item Id combo box (repairs + trash + pactcomponents TSVs, `quest_` items filtered out, free text allowed) and Count (integer > 0).
...
**Disassembly** — Dynamic entry list, identical in structure and controls to the weapon's Disassembly block (+ Add Entry, Item Id combobox, Count with the same integer > 0 validation). Saved to the JSON immediately after `Categories`. Ammo files created before this field existed will show it as a missing-field error on import; opening and saving the ammo in the editor adds it.

**Crafting Recipe tab** (after Localization) — identical to the weapon crafting editor, including the on/off switch (defaulting to **off** for ammo), except the Workshop Settings section is omitted entirely: just ProduceTimeInHours (number > 0, floats fine) and a Required Items list (max 7, same count validation, no info icon). Ammo receipts are byte-for-byte the same record structure as weapon receipts — named `{ammoId}_receipt.json`, `OutputItem` set to the ammo id, `Id` empty — but the unused workshop fields carry fixed inits: `ModifyStartCost` 1, `ModifyStep` 1, `ModifyItemsGrades` null, `ModifyLevelLimit` null. If a loaded file holds other values in those fields they pass through unchanged on save. Receipts are created with the ammo, and follow it through rename, copy, and delete.
...
- Disassembly (weapon config) — same text, beside the Add Entry / Copy Craft buttons
...
- **Errors** — block the import. Invalid JSON, a RecordType that doesn't belong in its folder, a filename that disagrees with the record's Id (datadisks are `{id}_diskData.json`, crafting recipes are matched on `OutputItem` since they never fill `Id`, must live in `Crafting Recipes/Weapons/` or `Crafting Recipes/Ammo/` — a recipe at the old `Crafting Recipes/` root is an error, and faction reward files must be `{FactionName}_factionData.json` with every `FactionRewardList` entry naming that same faction), invalid Id characters, duplicate Ids, collisions with base-game Ids (except datadisks, where matching an existing disk id is normal — the importer API appends to it), missing companion files (a weapon without its descriptor or localization, a firemode or explosion without its descriptor), orphaned companions with no parent record, localization keys under `Data.Keys` that don't match `item.{id}.*`, references to firemodes/ammo/traits that don't exist, traits used on the wrong item kind, files that aren't valid PNGs, and sprites that break the size rules (firemode 36×26, ammo floor/shadow 30×30 max). Faction reward files get an extra structural check: one faction per file, the filename must declare it (`{FactionName}_factionData.json`), and that faction must appear as a **single** entry whose `contentRecords` array holds all of its items. Splitting one faction across several entries is valid to the game but not to this tool — the editor rewrites the first matching entry only, so the extras would silently survive edits and deletions.

Every field the tool expects must be present — a record missing fields is an error, since an incomplete record can misbehave in game. All of a file's missing fields are reported on a single line rather than one error each. Field values are also type-checked against the same rules the editors enforce — a Radius that's text, a chance above 1, a Large Fire Chance above 100, an unknown damage type or weapon class, a negative weight, a disassembly count of zero, and so on. Also flagged: more than two firemodes or override-ammo entries on one weapon (the editor only has two slots, so extras would be lost), and a status effect that isn't one of the damage-renewal effects the ammo editor offers.
